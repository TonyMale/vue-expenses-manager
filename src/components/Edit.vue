			...
			<v-layout align-center justify-center row fill-height>
				<v-flex xs12 sm10 md8 lg6>
				  <v-card>
					...
					<v-form class="form__add" @submit.prevent = "onEditExpense">
...
<script>
	export default {
		...
		computed:{
			getExpense(){
				return this.$store.getters.getExpense(this.id)			
			}
		},
		methods:{
			onEditExpense(){
				const expenseData = {
					id: +this.getExpense.id,
					amount: +this.amount,
					date: new Date(this.date),
					description: this.description
				}
				this.$store.dispatch('editExpense', expenseData)
				this.$router.push('/')
			}
		},
		 created() {
         	this.description = this.getExpense.description
			this.date = new Date(this.getExpense.date).toISOString().slice(0,10)
			this.amount = this.getExpense.amount
        }
	}
</script>
